...
gameRoom.on('connect', async (socket) => {
  ...
  console.log('겜방연결후socket.nickname', socket.nickname);
  ...
  socket.on('disconnecting', async () => {
    ...
    try {
      ...
      const outPlayer = await Users.findOne({ id:socket.nickname }, { _id:false, state:true });
      if (outPlayer.state === 'blackPlayer'){
        await Users.updateOne({ id:socket.nickname }, { $set: { $inc: { 'score.0.win':1 } }});  //승 +1
        await Users.updateOne({ id:socket.nickname }, { $set: { point: point + 100 } });  //포인트 +100
      } else {
        await Users.updateOne({ id:socket.nickname }, { $set: { $inc: { 'score.1.lose':1 } }});  //패 +1
        await Users.updateOne({ id:socket.nickname }, { $set: { point: point - 50 } });  //포인트 -50
      }
