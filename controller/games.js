...
const gameFinishShow = async (req, res) => {
  try { //내id,  내게임방번호, 내게임승자id
    ...
    console.log("246,show,이긴팀 총포인트:",winTotalPoint)
    console.log("247,show,진팀 총포인트:",loseTotalPoint)
    ...
    console.log("276,show,winPlayer:",winPlayerArray)
    console.log("277,show,losePlayer:",losePlayerArray)
    
    
    console.log("280,show,games:",gameUserIds)
    // observer- whitePlayer가 이겼을때
    let winObserverArray1 = [];
    let loseObserverArray1 = [];
    if (result.win === gameUserIds.whiteTeamPlayer) {
      console.log("285,show, 이긴화이트옵저버 계산")
      console.log("286,show, 이긴화이트플레이어 아이디:",result.win)
      console.log("287,show, 이긴화이트 gameUserIds.whiteTeamPlayer는:",gameUserIds.whiteTeamPlayer)
      if (user.state === 'whiteObserver' && findTeachingCnt !== 0) { // 훈수챗 안했을때도 계산됨
      // if (user.state === 'whiteObserver' && findTeachingCnt !== 0) {  훈수챗 했을때만 계산됨.
        console.log("290,show, 이긴화이트옵 계산")
        //이긴팀 포인트 업데이트
        const winObserver = { id:user.id, usePoint:usePoint, getPoint:getPoint, 
                                            totalPoint:winTotalPoint, state:user.state };
        winObserverArray1.push(winObserver);
        console.log("295,show, 이긴화이트옵 winObserverArray1은?", winObserverArray1);
      } else if (user.state === 'blackObserver' && findTeachingCnt !== 0) {
        console.log("297,show,  진블랙옵 계산")
        //진팀 포인트 업데이트
        const loseObserver = { id:user.id, usePoint:usePoint, getPoint:getPoint, 
                                             totalPoint:loseTotalPoint, state:user.state };
        loseObserverArray1.push(loseObserver);
        console.log("302,show, 진블랙옵 loseObserverArray1은?", loseObserverArray1)
      }
    }
    ...
    if (result.win === gameUserIds.blackTeamPlayer) {
      console.log("309,show, 이긴블랙옵저버 계산")
      console.log("310,show, 이긴블랙플레이어 아이디:", result.win)
      console.log("311,show, 이긴블랙 gameUserIds.blackTeamPlayer는:", gameUserIds.blackTeamPlayer)
      if (user.state === 'blackObserver' && findTeachingCnt !== 0) {
        console.log("313,show, 이긴블랙옵 계산")
        //이긴팀 포인트 업데이트
        const winObserver = { id:user.id, usePoint:usePoint, getPoint:getPoint, 
                                            totalPoint:winTotalPoint, state:user.state };
        winObserverArray2.push(winObserver);
        console.log("318,show, 이긴블랙옵 winObserverArray2는?", winObserverArray2);
      } else if (user.state === 'whiteObserver' && findTeachingCnt !== 0) {
        console.log("320,show, 진화이트옵 계산")
        //진팀 포인트 업데이트
        const loseObserver = { id:user.id, usePoint:usePoint, getPoint:getPoint, 
                                            totalPoint:loseTotalPoint, state:user.state };
        loseObserverArray2.push(loseObserver);
        console.log("325,show, 진화이트옵 loseObserverArray2는?", loseObserverArray2);
      }
    } 
