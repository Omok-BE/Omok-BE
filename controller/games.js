  ...
  try {
    ...
    if (winPlayer.state === blackP.state) {
      ...
      //whiteObserver 짐
        ...
        console.log("174,화이트옵 진thisTeachingCnt리셋000:", thisTeachingCnt)
      }
    }
    res.status(200).json({
      ok: true,
...
const gameFinishShow = async (req, res) => {
  try { 
    ...
    console.log('194,결과창show진입:');
    console.log('195,결과창show,req.body:', req.body);

    //게임방내 유저 state별 정보
    const gameInfo = await gameUserInfo(gameNum);
    console.log("199,show,gameInfo[0]:",gameInfo[0]); 
    ...
    if (result.win === id) {
      ...
      console.log("221,gameFinishShow --> 이겼다~~!!winPlayerArray:",winPlayerArray);
      ...
      console.log("230,gameFinishShow --> 졌어요.....losePlayerArray:",losePlayerArray);
      ...
      for(let i=0; i<blackO.length; i++){
          ...
          if (getPoint >= 300){
            getPoint = 300
            console.log("243,show,getPoint:", getPoint)
          }
          ...
          console.log("251,show,win,blackO[i]",blackO[i])  
        }
      }
      console.log("254,show, 이긴블랙옵 winObserverArray1는?", winObserverArray1);
      ...
      for(let i=0; i<whiteO.length; i++){
        if (whiteO[i].state === 'whiteObserver') {
          ...
          if (getPoint >= 300){
            getPoint = 300
            console.log("267,show,getPoint:", getPoint)
          }
          ...
          console.log("275,show,lose,whiteO[i]",whiteO[i])  
        }
      } 
      console.log("278,show, 진화이트옵 loseObserverArray1는?", loseObserverArray1);
    } 
    ...
    if (result.win === whiteP.id) {
      console.log("285,Show,이긴화이트 플레이어는:", result.win)
      //화이트플레이어 승 계산
      ...
      const winInfo = {id:whiteP.id, usePoint:0, getPoint:getWPoint, existPoint:existWPoint,
                                          totalPoint:totalWPoint, state:whiteP.state };
      winPlayerArray.push(winInfo);    
      console.log("293,gameFinishShow --> 이겼다~~!!winPlayerArray:",winPlayerArray);
      ...
      const loseInfo = {id:blackP.id, usePoint:0, getPoint: - getBPoint, existPoint:existBPoint,
                                        totalPoint:totalBPoint, state:blackP.state };
      losePlayerArray.push(loseInfo);
      console.log("302,gameFinishShow --> 졌어요.....losePlayerArray:",losePlayerArray);
      ...
      for(let i=0; i<whiteO.length; i++){
        ...
        const addPoint = 20; //추가포인트
        let getPoint = usePoint + chatPoint + addPoint; //얻은포인트
        console.log("312,show,getPoint:", getPoint)
          if (getPoint >= 300){
            getPoint = 300
            console.log("315,show,getPoint:", getPoint)
          }
        ...
        console.log("323,show,win,whiteO[i]",whiteO[i])
        }
      }  
      console.log("326,show, 이긴화이트옵 winObserverArray2는?", winObserverArray2);
      ...
      for(let i=0; i<blackO.length; i++){
        if (blackO[i].state === 'blackObserver') {
          ...
          if (getPoint >= 300){
            getPoint = 300
            console.log("339,show,getPoint:", getPoint)
          }
          ...
          console.log("347,show,lose,blackO[i]:",blackO[i])
        }
      } 
      console.log("350,show, 진블랙옵 loseObserverArray2는?", loseObserverArray2);
    }  
    ...
    console.log("355,show,win배열 총정보:",win)
    console.log("356,show,lose배열 총정보:",lose)
    console.log("357,result",result)
...
const gameDelete = async (req, res) => {
  try {
    ...
    console.log("389,delete,existGame:",existGame)
    if (existGame){
      ...
      console.log("393,deleteRoomNum",deleteRoomNum)
      console.log("394,deleteGameNum",deleteGameNum)
    }
