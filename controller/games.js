...
const gameStart = async (req, res) => {
  try {
    ...
    console.log('50번gameInfo[0]:', gameInfo[0]);
...
const gameFinish = async (req, res) => {
  try {
    ...
    console.log("75,gameFinish,req.body:", req.body)
    ...
    console.log("84,winPlayer", winPlayer) 
    ...
    console.log("119,gameFinish,이긴옵저버 winTotalPoint:",winTotalPoint)

    //진팀 point
    const penalty = 20  //패널티
    let loseGetTeachingPoint = useTeachingPoint + chatPoint + penalty;
    console.log("124,gameFinish,진옵저버 최소포인트:",loseGetTeachingPoint) //얻은 포인트
    // 포인트제한두기- 최소포인트 300
    if(loseGetTeachingPoint >= 300){
      loseGetTeachingPoint = 300
      console.log("128,gameFinish,진옵저버 최소포인트:",loseGetTeachingPoint)
    } 
  
    const loseTotalPoint = point - loseGetTeachingPoint;  //게임후 총 포인트(기존 - 얻은)
    console.log("132,gameFinish,진옵저버 loseTotalPoint:",loseTotalPoint)

    //whitePlayer 이김
    if (winPlayer.state === 'whitePlayer') {
      console.log("화이트옵 이긴포인트계산- whitePlayer 이김")
      if (whiteObserverState === 'whiteObserver') {
        console.log("138, 화이트옵 이긴포인트계산 whitePlayer이겼을때")
        //포인트 업데이트
        await Users.updateOne({ id: id }, { $set: { point: winTotalPoint } });
        console.log("141,화이트옵 이긴thisTeachingCnt업뎃후리셋전:", thisTeachingCnt)
        console.log("142,화이트옵 이긴thisTeachingCnt리셋000:", thisTeachingCnt)
      } else if (blackObserverState === 'blackObserver') {
        console.log("144,블랙옵 진포인트계산 whitePlayer이겼을때")
        await Users.updateOne({ id: id }, { $set: { point: loseTotalPoint } });
        console.log("146,블랙옵 진thisTeachingCnt업뎃후리셋전:", thisTeachingCnt)
        console.log("147,블랙옵 진thisTeachingCnt리셋000:", thisTeachingCnt)
      }
    }
    ...
    if (winPlayer.state === 'blackPlayer') {
      ...
      if (blackObserverState === 'blackObserver') {
        console.log("156,블랙옵 이긴포인트계산 blackPlayer이겼을때")
        //포인트 업데이트
        await Users.updateOne({ id: id }, { $set: { point: winTotalPoint } });
        console.log("159,블랙옵 이긴thisTeachingCnt업뎃후리셋전:", thisTeachingCnt)
        console.log("160,블랙옵 이긴thisTeachingCnt리셋000:", thisTeachingCnt)
      //whiteObserver 짐
      } else if (whiteObserverState === 'whiteObserver') {
        console.log("163,화이트옵 진포인트계산 blackPlayer이겼을때")
        //포인트 업데이트
        await Users.updateOne({ id: id }, { $set: { point: loseTotalPoint } });
        console.log("166,화이트옵 진thisTeachingCnt업뎃후리셋전:", thisTeachingCnt)
        console.log("167,화이트옵 진thisTeachingCnt리셋000:", thisTeachingCnt)
      }
    }
    res.status(200).json({
      ok: true,
...
const gameFinishShow = async (req, res) => {
  try { 
    ...
    if (result.win === blackP.id) {
      console.log("206,Show,이긴블랙 플레이어는:", result.win)
      //블랙플레이어 승 계산
      ...
      console.log("214,gameFinishShow --> 이겼다~~!!winPlayerArray:",winPlayerArray);
      ...
      console.log("223,gameFinishShow --> 졌어요.....losePlayerArray:",losePlayerArray);
      ...
      for(let i=0; i<blackO.length; i++){
          ...
          console.log("233,show,getPoint:", getPoint)
          if (getPoint >= 300){
            getPoint = 300
            console.log("236,show,getPoint:", getPoint)
          }
          ...
          console.log("244,show,win,blackO[i]",blackO[i])  
        }
      }
      console.log("247,show, 이긴블랙옵 winObserverArray1는?", winObserverArray1);
      ...
      for(let i=0; i<whiteO.length; i++){
        if (whiteO[i].state === 'whiteObserver') {
          ...
          console.log("257,show,getPoint:", getPoint)
          if (getPoint >= 300){
            getPoint = 300
            console.log("260,show,getPoint:", getPoint)
          }
          ...
          console.log("268,show,lose,whiteO[i]",whiteO[i])  
        }
      } 
      console.log("271,show, 진화이트옵 loseObserverArray1는?", loseObserverArray1);
    } 
    ...
    if (result.win === whiteP.id) {
      console.log("278,Show,이긴화이트 플레이어는:", result.win)
      //화이트플레이어 승 계산
      ...
      const winInfo = {id:whiteP.id, usePoint:0, getPoint:getWPoint, existPoint:existWPoint,
                                          totalPoint:totalWPoint, state:whiteP.state };
      winPlayerArray.push(winInfo);    
      console.log("286,gameFinishShow --> 이겼다~~!!winPlayerArray:",winPlayerArray);
      ...
      const loseInfo = {id:blackP.id, usePoint:0, getPoint: - getBPoint, existPoint:existBPoint,
                                        totalPoint:totalBPoint, state:blackP.state };
      losePlayerArray.push(loseInfo);
      console.log("295,gameFinishShow --> 졌어요.....losePlayerArray:",losePlayerArray);
      ...
      for(let i=0; i<whiteO.length; i++){
        ...
        const addPoint = 20; //추가포인트
        let getPoint = usePoint + chatPoint + addPoint; //얻은포인트
        console.log("305,show,getPoint:", getPoint)
          if (getPoint >= 300){
            getPoint = 300
        ...
        console.log("316,show,win,whiteO[i]",whiteO[i])
        }
      }  
      console.log("319,show, 이긴화이트옵 winObserverArray2는?", winObserverArray2);
      ...
      for(let i=0; i<blackO.length; i++){
        if (blackO[i].state === 'blackObserver') {
          ...
          const penalty = 20; //진팀 패널티
          let getPoint = usePoint + chatPoint + penalty;  //얻은포인트
          console.log("329,show,getPoint:", getPoint)
          if (getPoint >= 300){
            getPoint = 300
            console.log("332,show,getPoint:", getPoint)
          }
          ...
          console.log("340,show,lose,blackO[i]:",blackO[i])
        }
      } 
      console.log("343,show, 진블랙옵 loseObserverArray2는?", loseObserverArray2);
    }  
    ...
    console.log("348,show,win배열 총정보:",win)
    console.log("349,show,lose배열 총정보:",lose)
    console.log("350,result",result)

    //Observer의 teachingCnt 0으로 리셋
    const delTeachingCnt = await Users.findOne({id},{_id:false, id:true, state:true, teachingCnt:true});
    console.log(",show,delTeachingCnt.id:",delTeachingCnt.id)
    console.log(",show,delTeachingCnt.teachingCnt:",delTeachingCnt.teachingCnt)
...
//[결과창]페이지로 들어가자마자
const gameFinishOutShow = async (req, res) => {
  try { 
    const { id, gameNum, result } = req.body;
    console.log('187,결과창show진입:');
    console.log('188,결과창show,req.body:', req.body);
...
const gameDelete = async (req, res) => {
  try {
    ...
    console.log("382,delete,existGame:",existGame)
    if (existGame){
      ...
      console.log("386,deleteRoomNum",deleteRoomNum)
      console.log("387,deleteGameNum",deleteGameNum)
    }
