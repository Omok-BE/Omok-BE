...
module.exports.participantUpdate = async ({ id, roomNum }) => {
  ...
  if (!gameInfo) {
    ...
    const blackPlayerUpdate = Rooms.updateOne(
      { roomNum },
      { $set: { blackTeamPlayer: null } }
    );

    const isBlackPlayer = Rooms.aggregate([
      {
        $match: { roomNum, blackTeamPlayer: id }
      }
    ])
    .then((blackPlayerUpdate) => {
      console.log(blackPlayerUpdate)
    })
    .catch((err) => {
      console.log(err)
    })

    const whitePlayerUpdate = Rooms.updateOne(
      { roomNum },
      { $set: { whiteTeamPlayer: null }}
    );

    const isWhitePlayer = Rooms.aggregate([
      {
        $match: { roomNum, whiteTeamPlayer: id }
      }
    ])
    .then((whitePlayerUpdate) => {
      console.log(whitePlayerUpdate)
    })
    .catch((err) => {
      console.log(err)
    })
